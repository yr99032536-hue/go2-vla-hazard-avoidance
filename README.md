...
마지막 액션 `action[7]`은 실제 모터를 움직이지 않는다. 대신 **골목 판정값**으로 사용하도록 학습시켰다. 모델이 직접 정수를 출력하지는 않고, **연속 실수**를 출력하면 실행기가 임계값으로 분류해 정수 신호를 만든다.

| 모델 출력 (연속 실수) | 실행기 분류 | 주행기 신호 |
|---|---|---|
| 예: -0.98 (≤ -0.5) | 위험 | -1 |
| 예: +0.97 (≥ +0.5) | 안전 | +1 |
| 예: 0.10 (-0.5 < x < 0.5) | 확인 중 | 0 |
...
실행기의 판정 게이트(`HazardDecisionGate`)는 이 연속값을 임계값 ±0.5 기준으로 분류하고, 같은 분류가 3번 연속 나와야 최종 신호로 주행기에 전달한다. 주행 제어에는 정수 신호(-1/+1)를 쓰고, 로그에는 원본 실수(`raw_decision`)를 함께 기록한다.
